...
// Helper function to get parameter name from widget
QString getParameterName(QWidget *widget) {
    ...
    // Get widget class name and object name
    QString className = widget->metaObject()->className();
    QString objectName = widget->objectName();
    
    // Build widget info
    if (!objectName.isEmpty() && !objectName.startsWith("qt_")) {
        widgetInfo = objectName;
    } else {
        widgetInfo = className;
    }
    
    // Try to find the label associated with this widget
    QWidget *parent = widget->parentWidget();
    QString labelText = "";
    
    while (parent && labelText.isEmpty()) {
        // Look for labels in the same parent
        QList<QLabel*> labels = parent->findChildren<QLabel*>();
        for (QLabel *label : labels) {
            // Check if this label is in the same layout as our widget
            QLayout *layout = parent->layout();
            if (layout) {
                for (int i = 0; i < layout->count(); ++i) {
                    QLayoutItem *item = layout->itemAt(i);
                    if (item && item->widget() == widget) {
                        // Found our widget, look for nearby labels
                        for (int j = 0; j < layout->count(); ++j) {
                            QLayoutItem *labelItem = layout->itemAt(j);
                            if (labelItem && labelItem->widget() == label) {
                                QString text = label->text();
                                if (!text.isEmpty() && !text.contains(":") && text.length() < 50) {
                                    labelText = text;
                                    break;
                                }
                            }
        ...
        // Also try to find labels that might be siblings
        QWidget *grandParent = parent->parentWidget();
        if (grandParent && labelText.isEmpty()) {
            QList<QLabel*> siblingLabels = grandParent->findChildren<QLabel*>();
            for (QLabel *label : siblingLabels) {
                // Check if this label is near our widget
    ...
    // Now try to find the effect name by looking up the widget hierarchy
    QWidget *currentWidget = widget;
    while (currentWidget) {
        QString widgetClass = currentWidget->metaObject()->className();
        QString widgetName = currentWidget->objectName();
        
        // Look for effect widget classes
        if (widgetClass.contains("Widget") && 
            (widgetClass.contains("Delay") || widgetClass.contains("Reverb") || 
             widgetClass.contains("Chorus") || widgetClass.contains("Flanger") ||
             widgetClass.contains("Phaser") || widgetClass.contains("Distortion") ||
             widgetClass.contains("Compressor") || widgetClass.contains("EQ") ||
             widgetClass.contains("Filter") || widgetClass.contains("Amp"))) {
            
            // Try to get the effect name from the widget
            if (!widgetName.isEmpty() && !widgetName.startsWith("qt_")) {
                effectName = widgetName;
            } else {
                // Extract effect name from class name
                QString className = widgetClass;
                if (className.contains("Delay")) effectName = "Delay";
                else if (className.contains("Reverb")) effectName = "Reverb";
                else if (className.contains("Chorus")) effectName = "Chorus";
                else if (className.contains("Flanger")) effectName = "Flanger";
                else if (className.contains("Phaser")) effectName = "Phaser";
                else if (className.contains("Distortion")) effectName = "Distortion";
                else if (className.contains("Compressor")) effectName = "Compressor";
                else if (className.contains("EQ")) effectName = "EQ";
                else if (className.contains("Filter")) effectName = "Filter";
                else if (className.contains("Amp")) effectName = "Amp";
            }
            break;
        }
    ...
    if (className.contains("SpinBox")) {
        ...
        if (spinBox) currentValue = QString::number(spinBox->value());
    } else if (className.contains("Slider")) {
        QSlider *slider = qobject_cast<QSlider*>(widget);
        if (slider) currentValue = QString::number(slider->value());
    } else if (className.contains("ComboBox")) {
        QComboBox *combo = qobject_cast<QComboBox*>(widget);
        if (combo) currentValue = combo->currentText();
    }
      ...
      cancelOperation(false), isInTransmissionState(false), isInImportState(false)
{
    ...
    // Test console output at startup
    ...
    if (logFile.open(QIODevice::WriteOnly | QIODevice::Append)) {
        ...
        fileStream << "=== MagicstompFrenzy SysEx Sniffer Started ===\n";
        fileStream.flush();
        logFile.close();
    }
...
void MainWindow::parameterChanged(int offset, int length, QWidget *paramEditWidget)
{
    ...
    // Log parameter change with name and SysEx data
    QString paramName = getParameterName(paramEditWidget);
    ...
    for (int i = 0; i < reqArr->size(); ++i) {
        stream << QString("%1").arg(static_cast<unsigned char>(reqArr->at(i)), 2, 16, QChar('0')).toUpper();
        if (i < reqArr->size() - 1) {
            stream << " ";
        }
    }
        ...
        QString logMessage = timestamp + " SYSEX OUT len=" + QString::number(reqArr->size())
                            + " | PARAM: " + paramName
                            + " (offset=" + QString::number(offset) + ", len=" + QString::number(length) + ")"
                            + "  " + hexString + "\n";
