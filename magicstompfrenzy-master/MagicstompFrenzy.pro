...
# Force console output on Windows - use AllocConsole

# Copy data files to output directory
win32 {
    copydata.commands = copy effects.ini $$OUT_PWD\\ && copy guitarpresets.ini $$OUT_PWD\\ && copy basspresets.ini $$OUT_PWD\\ && copy acousticpresets.ini $$OUT_PWD\\
...
unix {
    copydata.commands = cp effects.ini $$OUT_PWD/ && cp guitarpresets.ini $$OUT_PWD/ && cp basspresets.ini $$OUT_PWD/ && cp acousticpresets.ini $$OUT_PWD/
    first.depends = $(first) copydata
    export(first.depends)
    export(copydata.commands)
    QMAKE_EXTRA_TARGETS += first copydata
